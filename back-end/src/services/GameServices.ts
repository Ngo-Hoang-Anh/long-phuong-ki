...
export const timers = [timePerPlayer, timePerPlayer];
export class GameService {
  ...
  async updateGameById(
    ...
    if (gameData && gameData.isStart) {
      ...
      const timePlay = moment
        .duration(currentTime.diff(lastestTime))
        .asSeconds();
      const isTimeCalculateGreen =
        gameData.turn === "X" &&
        gameData.status === "setup" &&
        gameData.greenPrison.filter((prison) => prison.quantity === 1)
          .length === 0;
      const isTimeCalculateOrange =
        gameData.turn === "C" &&
        gameData.status === "setup" &&
        gameData.orangePrison.filter((prison) => prison.quantity === 1)
          .length === 0;
      if (
        ((data.turn === "C" || !data.turn) &&
          gameData.turn === "X" &&
          gameData.status === "play") ||
        isTimeCalculateGreen
      ) {
        // check update  turn
      ...
      if (
        ((data.turn === "X" || !data.turn) &&
          gameData.turn === "C" &&
          gameData.status === "play") ||
        isTimeCalculateOrange
      ) {
        // check update  turn
    ...
    gameData?.timers.forEach((item) =>
      item.time < 0 ? (item.time = 0) : item
    );
    return GameModel.findByIdAndUpdate(
      id,
      { ...data, timers: gameData?.timers },
      { new: true }
    );
  }
}
